...
function getSteps() {
  return ["Please Add Node Names", "Please Link The Nodes"];
}
...
function SetupStepper() {
  ...
  const getStepContent = (step, onAddLinks, onAddNodes) => {
    switch (step) {
      case 0:
        return (
          <AddNodeComponent onAddNodes={onAddNodes} deleteNode={deleteNode} />
        );
      case 1:
  ...
  const onAddLinks = (edges) => {
    ...
    let indx = copyLinks.findIndex((x) => x[0] === edges[0]);
    if (indx !== -1) {
      copyLinks.splice(indx, 1);
    }
    indx = copyLinks.findIndex((x) => x[1] === edges[0] && x[0] === edges[1]);
    if (indx !== -1) {
      return;
    }
  ...
  return (
    <div className={classes.root}>
      ...
      {resultDone ? (
        <D3Component id="graphTest" nodes={nodesReady} edges={linksReady} />
      ) : null}
    </div>
  );
}
